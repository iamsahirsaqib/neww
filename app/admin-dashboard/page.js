...
// Summary metrics (products, orders, revenue, reach)
...
// Chart data (empty initially — will be filled with REAL data only)
const [analyticsData, setAnalyticsData] = useState([]);

// Filtered chart data (no filtering — backend handles it)
const filteredAnalyticsData = useMemo(() => {
  return analyticsData;
}, [analyticsData]);

// --------------------------------------------
// FETCH ANALYTICS (REAL DATA ONLY)
// --------------------------------------------
const fetchAnalytics = async () => {
  try {
    ...
    if (data.success) {
      ...
      // Chart Data Only if exists
      if (Array.isArray(data.chart_data)) {

        const cleanedData = data.chart_data.map(item => ({
          ...item,
          // prevent NaN when visits = null or 0
          visits: item.visits ? Number(item.visits) : 0,
          orders: item.orders ? Number(item.orders) : 0,
          sales: item.sales ? Number(item.sales) : 0,
          revenue: item.revenue ? Number(item.revenue) : 0,

          // conversion % calculation (safe)
          conversion:
            item.visits > 0
              ? ((item.orders / item.visits) * 100).toFixed(2)
              : 0
        }));

        setAnalyticsData(cleanedData);
      }
    }
...
// Fetch when page loads or when active page changes
useEffect(() => {
...
{activePage === 'reviews' && (
  ...
  <div className="dashboard-analytics-dashboard">
    ...
    <div className="dashboard--metrics-summary-grid">
      ...
      <div className="dashboard-metric-card conversion-card">
        <h4 className="dashboard-metric-title">Conversion Rate</h4>
        <p className="dashboard-metric-value">
          {(() => {
            const totalVisits = filteredAnalyticsData.reduce((sum, item) => sum + item.visits, 0);
            const totalOrders = filteredAnalyticsData.reduce((sum, item) => sum + item.orders, 0);
    ...
    <div className="dashboard-charts-grid-layout">
      ...
      {/* Conversion Rate Chart (Changed from Ratio Chart) */}
      <div className="dashboard-chart-container ratio-chart">
        <h4 className="dashboard-chart-title">Conversion Rate %</h4>
        <ResponsiveContainer width="100%" height={300}>
